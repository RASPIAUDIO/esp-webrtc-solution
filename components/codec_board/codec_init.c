...
static int _i2s_init(uint8_t port, esp_codec_dev_type_t dev_type, codec_init_cfg_t *init_cfg)
{
    ...
    // Set loopback if both TX and RX handles exist for this port (after enabling channels)
    if (i2s_keep[port]->tx_handle && i2s_keep[port]->rx_handle) {
        ESP_LOGI(TAG, "Attempting to enable I2S loopback for port %d on tx_handle", port);
